...
# Initialize lists to store the mean reaction times for each condition
mean_reaction_times_condition1 = []
mean_reaction_times_condition2 = []
...
for row in x:
    for i, cell in enumerate(row):
        ...
        for trial in cell:
            ...
            crossing_time = np.argmax(trial >= 600)
            reaction_times.append(crossing_time)
        # Calculate the mean reaction time for the participant
        mean_reaction_time = np.mean(reaction_times)
        # Append the mean reaction time to the respective condition list
        if i == 0:
            mean_reaction_times_condition1.append(mean_reaction_time)
        else:
            mean_reaction_times_condition2.append(mean_reaction_time)

# Convert lists to numpy arrays for easier manipulation
mean_reaction_times_condition1 = np.array(mean_reaction_times_condition1)
mean_reaction_times_condition2 = np.array(mean_reaction_times_condition2)
...
# Histogram for condition 1
plt.subplot(1, 2, 1)
plt.hist(mean_reaction_times_condition1, bins=bins, edgecolor='black')
plt.axvline(np.mean(mean_reaction_times_condition1), color='red', linestyle='dashed', linewidth=1)
plt.title(f'Condition 1: Mean RT = {np.mean(mean_reaction_times_condition1):.2f} ms')
plt.xlabel('Reaction Time (ms)')
plt.ylabel('Frequency')

# Histogram for condition 2
plt.subplot(1, 2, 2)
plt.hist(mean_reaction_times_condition2, bins=bins, edgecolor='black')
plt.axvline(np.mean(mean_reaction_times_condition2), color='red', linestyle='dashed', linewidth=1)
plt.title(f'Condition 2: Mean RT = {np.mean(mean_reaction_times_condition2):.2f} ms')
plt.xlabel('Reaction Time (ms)')
plt.ylabel('Frequency')
...
# Print the values used in plotting
print("Mean RT Condition 1:", mean_reaction_times_condition1)
print("Mean RT Condition 2:", mean_reaction_times_condition2)

# Print all 60 values
print("All mean reaction times for Condition 1:")
for i, rt in enumerate(mean_reaction_times_condition1):
    print(f"Participant {i+1}: {rt}")

print("All mean reaction times for Condition 2:")
for i, rt in enumerate(mean_reaction_times_condition2):
    print(f"Participant {i+1}: {rt}")

# Check for normality using the Shapiro-Wilk test
shapiro_condition1 = shapiro(mean_reaction_times_condition1)
shapiro_condition2 = shapiro(mean_reaction_times_condition2)
...
if shapiro_condition1.pvalue > 0.05 and shapiro_condition2.pvalue > 0.05:
    # Both groups are normally distributed, use independent t-test
    test_stat, p_value = ttest_ind(mean_reaction_times_condition1, mean_reaction_times_condition2)
    test_name = 'Independent t-test'
else:
    # At least one group is not normally distributed, use Mann-Whitney U test
    test_stat, p_value = mannwhitneyu(mean_reaction_times_condition1, mean_reaction_times_condition2)
    test_name = 'Mann-Whitney U test'
